...
public class DictionaryController(IMediator mediator) : ApiControllerBase(mediator)
{
    ...
    public async Task<ActionResult<ApiResult<IEnumerable<string>>>> GetDictionary(DictionaryType dictionaryType, CancellationToken cancellationToken)
    {
        var result = await _mediator.Send(new GetDictionaryQuery() { DictionaryType = dictionaryType }, cancellationToken);
